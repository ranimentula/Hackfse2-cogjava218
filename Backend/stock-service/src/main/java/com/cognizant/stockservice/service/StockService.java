...
import com.cognizant.stockservice.client.RestClient;
...
import com.cognizant.stockservice.repository.StockRepository_JPA;

@Service
public class StockService {

	@Autowired
	private StockRepository_JPA stockRepository;
...
//	private MongoTemplate mongoTemplate;
	...
	public Stock saveStock(int companyCode, Stock stock) {
		try {
//			Company company = restTemplate.getForObject(COMPANY_URL+"info/" + companyCode,
//					Company.class);

			URI determinedBasePathUri = URI.create(COMPANY_URL);
			Company company = client.getCompany(determinedBasePathUri, companyCode);
...
//		}
	...
	public ResponseTemplateVO getStocksByDate(int companyCode, LocalDate date1, LocalDate date2) {
		...
		try {
//			company = restTemplate.getForObject(COMPANY_URL+"info/" + companyCode, Company.class);

			URI determinedBasePathUri = URI.create(COMPANY_URL);
			company = client.getCompany(determinedBasePathUri, companyCode);
...
//************use this method with mongo db and mongo template**********
